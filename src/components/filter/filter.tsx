...
const AppFilter = ({ ...props }) => {
    ...
    const onSearchChangeHandler = (value: string, event: { preventDefault: () => void; }) => {
    ...
    const onTypeChangeHandler = (value: string | any[], event: { preventDefault: () => void; }) => {
        event.preventDefault();
        if (value.length) {
            props.isFilterEnable(true);
    ...
    const onGenderChangeHandler = (value: string | any[], event: { preventDefault: () => void; }) => {
        event.preventDefault();
        if (value.length) {
            props.isFilterEnable(true);
            getAllParallelCall(value).then(pokemonList => {
    ...
    const filterPokemonData = (data: never[]) => {
        dispatch({
    ...
    const setPokemonTypes = (data: any[]) => {
        ...
        if (data.length) {
            data = data.map((item: { name: string | undefined; url: any; }) => ({ label: getCamleCaseString(item.name), value: item.url, url: item.url }));
            dispatch({
    ...
    const setPokemonGendersList = (genderList: any[]) => {
        genderList = genderList.map((item: { name: string | undefined; url: any; }) => ({ label: getCamleCaseString(item.name), value: item.url, url: item.url }));
